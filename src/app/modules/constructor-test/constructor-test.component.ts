...
export class ConstructorTestComponent {
  ...
  const BUTTON_SIZE = this.SWIMLANE_HEADER_HEIGHT - this.BUTTON_BORDER_OFFSET * 2;
  const POOL_MARGIN = 50;
  const SIGNS = {
    "+": `M ${this.BUTTON_SIZE / 2} 4 V ${this.BUTTON_SIZE - 4} M 4 ${this.BUTTON_SIZE / 2} H ${
      this.BUTTON_SIZE - 4
    }`,
    "-": `M 4 ${this.BUTTON_SIZE / 2} H ${this.BUTTON_SIZE - 4}`
  };
  const SWIMLANE_SIZE = {
    width: 450,
    height: 1750,
